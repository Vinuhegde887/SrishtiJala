    ...
    SidebarProps) => {
        ...
        const handleRollback = async (versionId: string)=>{
            try{
                ...
                const {data:data2} = await api.get(`/api/usser/project/rollback/${project.id}`);
    ...
    transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden':'w-full' }`}>
        <div className='flex flex-col h-full'>
            ...
            <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
                        ...
                        return (
                            <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end":"justify-start"}`}>
                                ...
                                <div className={`max-w-[80%] p-2 px-4 rounded-2xl
                                    ...
                                    "bg-linear-to-br from-indigo-600 to-indigo-700 text-white rounded-tr-none": "rounded-tl-none bg-gray-800 text-gray-100"}`}>
                                    {msg.content}
                                </div>
                                {isUser &&(
                ...
                {
                    isGenerating && (
                        <div className='flex items-tart gap-3 justify-start'>
            ...
            <form  onSubmit={handleRevisions} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    ...
                    value={input} rows={4} placeholder='Describe your website or request Changes..' 
                    className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring
                    ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400
                    transition-all' disabled={isGenerating}/>
                    <button disabled={isGenerating || !input.trim() } 
                    className='absolute bottom-2.5 right-2.5
                    rounded-full bg-linear-to-r from-indigo-500 to-indigo-600
                    hover:from-indigo-600 hover:to-indigo-700 text-white
                    transition-colors disabled:opacity-60'>
                        {isGenerating ? <Loader2Icon className='size-7 p-1.5 aniamate-spin text-white'/>
