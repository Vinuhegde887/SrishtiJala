 ...
 */
export function ChangePassword({ className, classNames }: ChangePasswordProps) {
  ...
  if (!isAccountsPending && !hasCredentialAccount) {
    return <SetPassword className={className} classNames={classNames} />
  }
...
function SetPassword({
  ...
  className?: string
  classNames?: ChangePasswordProps["classNames"]
}) {
  ...
  return (
    <div>
      <h2 className="w-full text-center text-sm font-semibold mb-3">
    ...
    <div>
      ...
      <form.AppForm>
        <form.AuthFormRoot>
          <Card className={cn(className, classNames?.base)}>
            <CardContent
              className={cn("flex flex-col gap-6", classNames?.body)}
            >
              <form.AppField
